...
import { validateUserChatData } from 'src/common/chat-validation';
...
export class AuthService {
  ...
  async uploadFromExcel(user: User, file: Express.Multer.File) {
    ...
    // Validate the fields and throw error if fails
    userChatData.forEach((d) => validateUserChatData(d));
    ...
    if (allSenderUserExists.length !== uniqueEmails.length) {
      throw new BadRequestException(
        'One or more sender users does nopt exists',
      );
    }

    const chatHistories = userChatData.map((d) => {
      const senderUser = allSenderUserExists.find(
        (u) => u.email === d.senderEmail,
      );
      this.em.persist(
        new ChatHistory(
          user,
