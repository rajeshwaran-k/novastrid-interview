## Tasks:

- User Registration - Completed
- Login API (Token Generation) - Completed
- Chat Import via Excel Sheet - Partially Completed (Validating data is pending)

## Setting up

- After clonning the application locally run the following command

```
npm run start:dev
```

- Application will be running in port 3000
- Import `postman-sample-request.json` into your Postman application. You will be able to see sample requests for login and signup.
- Use `http://localhost:3000/auth/chat/bulkupload` to upload excel sheet. You can also find a `sample-sheet.xlsx` in this repo.
